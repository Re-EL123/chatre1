...
(function () {
  ...
  const SLASH_COMMANDS = [
    ...
    { cat: "model", cmd: "/model", desc: "Show Chatre + BYOK models", action: () => {
      ...
      opts.forEach((o) => {
        ...
        if (/^(openrouter|aihubmix|zai|groq|deepseek|modelscope|ollama|kilo|cloudflare|llm7|ovhcloud|huggingface|dashscope|mistral|xai|anthropic|openai|google|cursor|byok):/i.test(v) || /byok/i.test(lab)) {
          byok.push("- `" + v + "` — " + lab);
        } else {
          chatre.push("- `" + v + "` — " + lab);
        }
  ...
  function isByokModel(model) {
    return /^(openrouter|aihubmix|zai|groq|deepseek|modelscope|ollama|kilo|cloudflare|llm7|ovhcloud|huggingface|dashscope|mistral|xai|anthropic|openai|google|cursor):/i.test(String(model || ""));
  }
